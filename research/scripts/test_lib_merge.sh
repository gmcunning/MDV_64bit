...
if [ "$1" = "debug" ]; then
  ...
  echo "usage: test_lib_merge.sh [debug|opt]"
  exit 1
fi
...
ifi_lib_src="/home/cunning/git/ral-ifi-libs"
...
cd $ifi_lib_src/Fmq || exit
make install_include || exit
cd $ifi_lib_src/dsdata || exit
make install_include || exit
cd $ifi_lib_src/toolsa || exit
make install_include || exit
cd $ifi_lib_src/rapformats || exit
make install_include || exit
cd $ifi_lib_src/Ncxx || exit
make install_include || exit
cd $ifi_lib_src/Mdv || exit
make install_include || exit
cd $ifi_lib_src/Spdb || exit
make install_include || exit
cd $ifi_lib_src/dsserver  || exit
make install_include || exit
