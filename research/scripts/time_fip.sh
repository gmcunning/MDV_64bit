...
# simple script to time running the FIP process chain
...
# run Grib2toMdv
echo timing for Grib2toMdv
time $g2m_app -params "$g2m_param" -f $g2m_infile >& /dev/null

# run Hybrid2Pressure
echo timing for Hybrid2Pressure
time $h2p_app -params "$h2p_param" -if $h2p_infile >& /dev/null

# run MdvDeriveModel
echo timing for MdvDeriveModel
time $m2m_app -params "$m2m_param" -if $m2m_infile >& /dev/null

# run fip_algo
echo timing for fip_algo
time $fa_app  -params "$fa_param" -if $fa_infile >& /dev/null

# run pressure2flight
echo timing for pressure2flight
time $p2f_app -params "$p2f_param" -if $p2f_infile >& /dev/null

# run IcingCategory
echo timing for IcingCategory
time $ic_app -params "$ic_param" -if $ic_infile >& /dev/null

# run MdvtoGrib2
echo timing for MdvtoGrib2
